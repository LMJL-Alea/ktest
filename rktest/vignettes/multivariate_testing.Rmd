...
Load `rktest` and install python dependency
```{r setup-rktest, eval=FALSE}
library(rktest)
install_pyktest(method = "virtualenv", envname = "ktest")
```

```{r setup-dev, include=FALSE}
proj_root_dir <- rprojroot::find_root(".git/index")
devtools::load_all(file.path(proj_root_dir, "rktest"))
```
...
```{r data}
sc_df <- read.table(
    "data/data2.csv", row.names = 1, sep = ",", header = TRUE)

meta_sc_df <- read.table(
    "data/metadata2.csv", row.names = 1, sep = ",", header = TRUE)
```
...
```{r multi}
head(kt$multivariate_test(verbose=1))
# OR
head(multivariate_test(kt, verbose=1))
```
