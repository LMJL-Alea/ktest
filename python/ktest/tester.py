...
class Ktest(Statistics):
    ...
    ----------
        ...
        metadata : 1-dimensional array-like
            ...
            library performs a two sample test only, thus exactly two levels 
        ...
        sample_names : None or 1-dimensional array-like, optional
            If None, sample names are derived from the metadata levels.
            If array-like, should containt two strings, labeling the samples
            (overrides the metadata levels).
    ...
    def __str__(self):
        ...
        ncs = 'not computed, run ktest.multivariate_test.'
    ...
    def compute_pvalue(self, stat='kfda', permutation=False, n_permutations=500,
        ...
        if stat == 'kfda' and not permutation:
            ...
            for i in it:
                ...
                if self.data_nystrom is not None:
                    ny_ind = list(self.data_nystrom.index.values())[0]
                    for ind in list(self.data_nystrom.index.values())[1:]:
                        ny_ind = ny_ind.union(ind)
                    data_perm_nystrom = Data(data=self.dataset.loc[ny_ind], 
                                             metadata=meta_perm.loc[ny_ind], 
                                             sample_names=self.sample_names)
