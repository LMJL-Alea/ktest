...
> :warning: For package installation and setup, see the dedicated vignette `vignette("install_ktest", package = "ktest")`. :warning:
...
You can directly manipulate Python kernel test objects in R thanks to `reticulate` using a `$` character instead of a `.` character to access the `Ktest` class attributes and member functions, e.g.:
...
You can compute the related Gram matrix with the following (note the `$numpy()` to convert it from a `torch.tensor` to a standard `numpy.array` which is automatically converted to an R matrix):
```{r gram, eval=FALSE}
kt_1$kstat$compute_gram()$numpy()
```

Then, you can compute its eigen decomposition with:
```{r diag_gram, eval=FALSE}
res <- kt_1$kstat$diagonalize_centered_gram()
res[[1]]$numpy() # eigen values
res[[2]]$numpy() # eigen vectors
```

In practice the eigen decomposition is also directly available after a call to the `test()` function (c.f. [above](#kernel-based-two-sample-test)):
```{r diag_gram2, eval=FALSE}
...
Since `ktest` R package is basically a wrapper around the `ktest` Python package, we manipulate Python objects directly in R:
```{r class, eval=FALSE}
class(kt_1)
...
Such type of objects cannot be saved and loaded using the standard `save()` and `load()` function in R.

You need to use the dedicated `save_ktest()` and `load_ktest()` functions from the package to do so:
