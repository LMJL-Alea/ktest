...
required <- c("conflicted", "reticulate")
sapply(
    required,
    function(pkg) {
...
> :warning: For package installation and setup, including Python environment configuration, see the dedicated vignette `vignette("install_ktest", package = "ktest")`.
...
```{r}
kt_1 = ktest(
    data = data_tab, metadata = meta_tab, 
...
```{r statistics}
get_statistics(kt_1, stat = 'kfda', contrib = FALSE)
```
...
```{r p-values}
get_pvalues(kt_1, stat = 'kfda', permutation = FALSE)
```
...
> **Note**: this plot generation system will work in Rmarkdown rendering but generally not (or not as expected) in console mode or in Rstudio. In that case, saving the figure should still work.
...
```{r plot_density}
fig <- kt_1$plot_density(t = 2L)
# plot figure (should work in Rmarkdwon but not in console mode or in Rstudio)
fig[[1]]
fig[[2]]
plt$show()
```
...
```{r projection_scatterplot}
fig <- kt_1$scatter_projection(t_x = 1, t_y = 2, proj_xy = c('kfda_contrib', 'kfda_contrib'))
# plot figure (should work in Rmarkdwon but not in console mode or in Rstudio)
fig[[1]]
fig[[2]]
plt$show()
...
```{r projection_contrib_scatterplot}
fig <- kt_1$scatter_projection(t_x = 1, t_y = 1, proj_xy = c('kfda', 'kfda_contrib'))
# plot figure (should work in Rmarkdwon but not in console mode or in Rstudio)
fig[[1]]
fig[[2]]
plt$show()
